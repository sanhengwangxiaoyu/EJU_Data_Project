...
#                                   urgent         迫切
#                                   cre            增存
browse_tag = con.query('''select imei,concern,intention,urgent,cre from dws_db.dws_imei_browse_tag where period = "'''+date_quarter+'''"''')
# 合并表
...
########################===============季度=============#######################
# 意向人数
df1_intention = df[df['concern'] == '关注']
df1_intention = df1_intention[['newest_id','imei']].drop_duplicates()
...
# 意向人数
# 意向人数
df2_intention = df[df['concern'] == '关注']
df2_intention = df2_intention[['newest_id','visit_month','imei']].drop_duplicates()
