...
/**
  * 0. 首先需要清空临时表
  *
  * truncate table qyf_tmp.t11;
  * truncate table qyf_tmp.t22;
  *
  *
  * 1. querySQL1中的九个字段需要都跑一遍
  *
  * 2. 计算并入库
  * insert into  dws_db.dws_tag_purchase_prefer
  * (period,newest_id,city_id,tag_value,value1,value2,value3,value4,tag_name,value5)
  * select if(t3.period=1,'2021Q1',if(t3.period =4,'2020Q4',t3.period))as period ,t3.newest_id ,t3.city_id ,t3.tag_value ,t3.value1 ,t3.value2,t4.value3 ,t4.value4 ,t4.tag_name ,(t3.value1 /t3.value2 )/(t4.value3/t4.value4) as rate
  * from qyf_tmp.t11 t3
  * left join qyf_tmp.t22 t4
  * on t3.period =t4.period and t3.city_id =t4.city_id and t3.tag_value = t4.tag_value ;
  */
object SocialPerfer {
  def main(args: Array[String]): Unit = {
    ...
    val querySQL1 =
      """
        |SELECT imei,social_prefer,concat_ws ('',social_prefer,read_prefer,game_prefer,live_prefer,
      ...
      .apply(
        (l, f) => {
            ...
            (l._1, //imei
              ...
              if (f.getField(1) !=null &&f.getField(2) !="")"社交类型" else "", //social_prefer
