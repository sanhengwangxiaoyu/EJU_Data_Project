...
SELECT city_id,imei,newest_id,visit_date FROM dwb_db.dwb_customer_browse_log
''')
...
# 统计迫切与意向
res,columnNames = con.query('''
select * from dws_db.dws_imei_browse_tag_tmp
'''
)
...
res,columnNames = con.query('''
 select newest_id,city_id,county_id from dws_db.dws_newest_info
''')
