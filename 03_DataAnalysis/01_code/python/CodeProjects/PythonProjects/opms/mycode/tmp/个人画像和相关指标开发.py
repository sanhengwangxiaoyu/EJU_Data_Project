...
week_date = start_date_DF.strftime("%Y-%W")
...
#                                                      newest_id      楼盘id
#                                                      pv             浏览次数  
ori=con.query('''SELECT imei,visit_date date,newest_id,pv FROM dwb_db.dwb_customer_browse_log where visit_date>='''+start_date+''' and visit_date<'''+stop_date+''' limit 1000''')
...
# In[20]:
# odsdb.cust_browse_log_*   客户浏览日志表
#                                          customer      用户号码
#                                          idate         数据接收日期
#                                          city_name     城市名称
#                                          floor_name     楼盘或小区
...
# ''')
#imei标签-增存
las=con.query("SELECT distinct customer imei,concat(substr(idate,1,4),"-",substr(idate,5,2),"-",substr(idate,7,2)),city_name,floor_name,pv FROM odsdb.cust_browse_log_202004_202103 where date_format(idate,'%Y-%m-%d') >= '"+pre_start_date+"'  and date_format(idate,'%Y-%m-%d') <'"+pre_end_date+"' limit 1000")
...
#              增量客户：本周/月之前未浏览本楼盘为增量客户，统一周期为半年
