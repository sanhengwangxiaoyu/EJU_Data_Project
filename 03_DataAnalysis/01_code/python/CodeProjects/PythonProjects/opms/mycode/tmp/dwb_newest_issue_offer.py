...
# else:
issue_offer=con.query("select url ,gd_city ,floor_name ,business ,address ,issue_code , issue_date_clean ,replace(substr(issue_date_clean,1,7),'-','') issue_month , '"+date_quarter+"' issue_quarter ,case when room_sum is null then 0 when room_sum = '' then 0 when room_sum = 'None' then 0 else room_sum end room_sum,building_code ,case when issue_area = 'None' then 0 when issue_area = '' then 0 else substr(issue_area,1,8) end issue_area from odsdb.city_newest_deal where issue_code is not null and issue_code != '' and issue_date_clean between '"+start_date+"' and '"+end_date+"'")
...
# ########################################################################
# ########################################################################
# #按照room_sum统计一份套数 （深圳 佛山 嘉兴 南宁 九江 保定 长沙 东莞 海口 合肥 青岛 沈阳 厦门 淄博）
# #   深圳市
...
# # In[9]:
# ###合并结果集
# # 深圳 佛山 嘉兴 南宁 九江 保定 长沙 东莞 海口 合肥 青岛 沈阳 厦门 淄博 无锡市 成都 贵阳 惠州 武汉 济南  肇庆 
# reult1 = pd.concat([df_zq,df_jn,df_wh,df_hz,df_gy,df_cd,df_wx,df_zb,df_xm,df_sheny,df_qd,df_hf,df_hk,df_dg,df_cs,df_bd,df_jj,df_fs,df_nn,df_jx,df_sz])
