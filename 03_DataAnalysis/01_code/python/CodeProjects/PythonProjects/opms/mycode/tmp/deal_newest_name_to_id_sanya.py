...
# 获取当前城市的所有预售证信息
issue_offer=con.query("select url ,gd_city ,floor_name newest_name,business developer,case when address='' then '暂无' else address end address,issue_code , issue_date_clean ,replace(substr(issue_date_clean,1,7),'-','') issue_month , '"+date_quarter+"' issue_quarter ,case when room_sum is null then 0 when room_sum = '' then 0 when room_sum = 'None' then 0 else room_sum end room_sum,building_code ,case when issue_area = 'None' then 0 when issue_area = '' then 0 else substr(issue_area,1,8) end issue_area ,room_code from odsdb.city_newest_deal where issue_code is not null and issue_code != '' and issue_date_clean between '"+start_date+"' and '"+end_date+"' and city_name = '三亚'")
...
##去除脏数据
df_2 = df_2[~df_2['formal_newest_name'].isnull()]
###    直接根据公司名关联上的
...
#In[]
##预售证和楼盘id合并
result = (pd.merge(df1,df_sy,how='left',on=['newest_name'])).drop_duplicates(inplace=False).reset_index()
##去重
# result = result.groupby(['newest_name','','developer','gd_city','issue_code','issue_date_clean','issue_month','issue_quarter','issue_area','room_sum'])['url'].max().reset_index()
...
# newest_name.to_csv('C:\\Users\\86133\\Desktop\\newest_name.csv')
# result
print('>>>>>>>Done')
