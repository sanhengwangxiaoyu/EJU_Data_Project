...
#  删除resi_province这一列的数据 df3.drop('resi_province',axis=1)  
#  在resi_province 的值是 ("上海市,安徽省") 类似中间有逗号的时候，通过split切分，并保留两条数据（一行变两行）df3['resi_province'].str.split(.... ）
#  讲拆分后的和拆分前两列数据拼接在一起join  
...
# 新建一个area_type字段，赋值area_province
...
# 对类数据按照逗号进行拆分         --,海南省-陵水黎族自治县-陵水黎族自治县
...
# 对这类数据按照横岗拆分并且只取最后一个值      海南省-陵水黎族自治县-陵水黎族自治县
...
# 拼接用户浏览楼盘记录与常驻区域：根据imei关联，将楼盘id，地区id，地区名字拼接在一起
dfre = pd.merge(df2,df101,how='inner',on='imei')
# 按照每个楼盘，城市，区县，季度作为窗口统计浏览人数
