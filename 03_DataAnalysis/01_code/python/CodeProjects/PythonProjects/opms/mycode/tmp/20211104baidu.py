...
baidu = con.query("SELECT newest_id,concat(lat,',',lng) lat_lng from city_detail_baidu")

newest_ls = baidu.lat_lng.drop_duplicates()
newest_ls = list(newest_ls)
gd_lat_lng = pd.DataFrame()

for i in newest_ls:
    print(i)
    try:
        url = 'https://restapi.amap.com/v3/assistant/coordinate/convert?parameters'   # 输入API问号前固定不变的部分
        ...
        res.text
        jd = json.loads(res.text)  
        jd    # 将json数据转化为Python字典格式
        baidu_locations = i
        location = jd['locations']
        gd = pd.DataFrame([i])
        gd['gd_lat_lng'] = pd.DataFrame([location])
        gd_lat_lng = gd_lat_lng.append(gd)
    except :''
gd_lat = gd_lat_lng.merge(baidu,how='left',left_on=0,right_on='lat_lng')
gd_lat[['gd_lat','gd_lng']] = gd_lat['gd_lat_lng'].str.split(',',expand=True)
gd_lat.drop(0,axis=1,inplace=True)

to_dws(gd_lat,'gd_lat_lng')
