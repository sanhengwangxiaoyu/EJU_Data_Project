...
####依赖  auth_user!!!!!!!!!!!!!!!!!!!!!
web_pages = con.query("select ods_id,concat('"+yesterday_date_time+"',' ',LPAD(case when substr(create_time,12,2) = 16 then '00' else substr(create_time,12,2)+8 end,2,'0') ,substr(create_time,14,6))as date_time,url,app_mobile register_mobile,track_behaviorTag from odsdb.ods_gd_points_web_pages where create_time >= '"+pages_date_time+"' and create_time <'"+pages_yesterdaydate_time+"' and app_mobile is not null and dr = 0 group by ods_id,create_time,url,app_mobile,track_behaviorTag")#用户浏览url
...
df_etday_report.columns=['invite_number','sum_cust_num','new_cust_num_id','register_time']#修改列名
df_etday_report = pd.merge(auth_user_invite,df_etday_report,how='left',on=['invite_number'])#关联code
...
df_day_report = pd.merge(df_day_report_today,df_day_report_browae,how='left',on=['user_id'])#两个标签合并
# 最近三天是否有登录
...
df_day_report.at[~df_day_report['user_id'].isin(df_day_report_userlog['user_id']),'lost_index']=1#打标签
#######各渠道总登录用户
...
df_report_result.at[df_report_result['new_cust_num_id']==0,'new_cust_num_id']=np.nan
# df_report_result
df_report_result = df_report_result.rename(columns={'register_user_id':'sum_cust_num','new_cust_num_id':'new_cust_num','url_model':'show_report_cust_num','buy_first_report':'active_cust_num','buy_thired_report':'active_super_cust_num','lost_index':'lost_cust_num'})  #修改列名
