...
"""
issue_offer=con.query("select newest_id ,max(id) id from dws_db_prd.dws_tag_purchase_poi where pure_distance<=3 group by newest_id")
issue_offer.columns=['newest_id','id']
...
#In[]
df_in = newest_id[~newest_id['newest_id'].isin(df['newest_id'])]
df_in['id'] = 0
...
# update_sql1 = "update odsdb.ori_newest_period_admit_info set dr = 1 where dr = 0"
# cur.execute(update_sql1)
# conn.commit() # 提交记
# conn.close() # 关闭数据库链接
# print('>>> truncat table Done')


# In[10]:
# 加载到新表 ori_newest_period_admit_info
# to_dws(df_result_0,'ori_newest_period_admit_info')


#In[]
###先将待爬楼盘表数据清空。使用python将数据加载到表里并且加载到txt文件中
df_result_0_1 = df_result_0.head(700)
df_result_0_2 = df_result_0[~df_result_0['newest_id'].isin(df_result_0_1['newest_id'])]
df_result_0_1.to_csv(path_or_buf='C:\\Users\\86133\\Desktop\\search_gps.txt',sep='\t',columns=['newest_name','address','lnglat'],header=False,index=False)
df_result_0_2.to_csv(path_or_buf='C:\\Users\\86133\\Desktop\\search_gps_v3.txt',sep='\t',columns=['newest_name','address','lnglat'],header=False,index=False)
