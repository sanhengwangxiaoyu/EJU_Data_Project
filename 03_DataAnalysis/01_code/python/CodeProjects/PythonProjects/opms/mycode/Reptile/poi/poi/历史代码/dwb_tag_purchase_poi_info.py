...
# In[]:
newest=con.query("select min(id) newest_ods_id,newest_name,newest_id,address newest_address,concat(substring_index(lnglat,',',1)+0,',',substring_index(lnglat,',',-1)+0) newest_lnglat,city_id from odsdb.ori_newest_period_admit_info where dr = 0 group by newest_name,newest_id,address,lnglat,city_id")
...
df = pd.merge(poi,newest,how='inner',on=['newest_name','newest_address'])
# poi[~poi['newest_name'].isin(df['newest_name'])]
...
result = result[['newest_ods_id','poi_ods_id','newest_id','newest_lnglat_y','poi_type','poi_index','poi_name','poi_lnglat','pure_distance','dr','create_time','update_time','city_id']]
result.columns = ['newest_ods_id','poi_ods_id','newest_id','newest_lnglat','poi_type','poi_index','poi_name','poi_lnglat','pure_distance','dr','create_time','update_time','city_id']
