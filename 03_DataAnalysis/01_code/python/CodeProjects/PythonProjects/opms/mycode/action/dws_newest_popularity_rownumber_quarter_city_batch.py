...
opts,args=getopt.getopt(sys.argv[1:],"t:q:s:e:d:c:",["cityid","database=","table=","quarter=","startdate=","enddate="])
...
newest_id=con.query("select newest_id,city_id,county_id from dws_db_prd.dws_newest_info where newest_id is not null and city_id in ('110000','120000','310000','500000') and county_id is not null and county_id != '' group by newest_id,city_id,county_id")


# In[3]:
# 获取指定季度的楼盘id,和城市id，区县id
newest_admit = pd.merge(admit,newest_id, how='inner', on=['newest_id'])
# 通过楼盘id，获取浏览的客户数量
