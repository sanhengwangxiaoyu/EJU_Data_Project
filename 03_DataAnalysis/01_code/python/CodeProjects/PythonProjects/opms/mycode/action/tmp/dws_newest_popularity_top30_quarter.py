...
table_name = 'dws_newest_popularity_top30_quarter'
...
# ori = ori.drop_duplicates(['newest_id','imei'])
ori=ori.groupby(['newest_id']).agg({'imei':pd.Series.nunique}).reset_index()
...
# 取出每个城市客户数是前30的数据
grouped = ori1.groupby(['city_id']).head(1000000)
...
# 前30
grouped0 = ori2.groupby(['county_id']).head(1000000)
# 计算每个区域的客户总数
