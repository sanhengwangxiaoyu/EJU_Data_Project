...
#                                   urgent         迫切
#                                   cre            增存
ime = con.query(" SELECT max(ods_id) id,min(add_new_code) cre,visit_month period FROM dwb_db.dwb_customer_add_new_code where dr = '0' and visit_quarter = '"+date_quarter+"' group by imei,visit_month")
