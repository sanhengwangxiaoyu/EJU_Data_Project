...
"""
import configparser, os, pymysql, pandas as pd, numpy as np, re
...
正式代码：
  逻辑如下：
    ...
    在意向的基础上,那批人的(用户关注楼盘的最大值-自己关注的中位数)/自己关注的中位数>180% 则为迫切用户
...
#imei标签-三度
ori=con.query(" SELECT city_id,newest_id,pv,visit_date,imei, '"+period+"' period FROM dwb_db.a_dwb_customer_browse_log where newest_id = '"+newest_id+"' and visit_date>='"+start_date+"' and visit_date<='"+end_date+"' ")


# In[5]:
imeis = "'"+re.sub(r',','\', \'',','.join(ori['imei'].values.tolist()))+"'"
add_new_code = con.query(" SELECT imei,min(add_new_code) cre,visit_quarter period FROM dwb_db.dwb_customer_add_new_code where imei in ("+imeis+") and  dr = '0' and visit_quarter = '"+period+"' group by imei,visit_quarter")
...
#迫切
imei_browse_tag.at[(imei_browse_tag['intention'] == '意向')&((imei_browse_tag['max_cu']-imei_browse_tag['median_cu'])/imei_browse_tag['median_cu']>1.8),'urgent']='迫切'
...
had_imeis = con.query("select imei from dws_db_prd.dws_imei_browse_tag where imei in ("+imeis+")  and period = '"+period+"'")
nohad_imeis = imei_browse_tag[~imei_browse_tag['imei'].isin(had_imeis['imei'])]


# In[9]:
#imei_browse_tag.columns=['period','customer','concern','intention','urgent','cre']
to_dws(nohad_imeis,table_name)
print('>>> Done')
