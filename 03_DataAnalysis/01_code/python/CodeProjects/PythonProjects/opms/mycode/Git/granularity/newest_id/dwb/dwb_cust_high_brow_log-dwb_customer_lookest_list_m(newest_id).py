...
for period in periods:
    ...
    """
    res,columnNames  = con.query("select newest_id,min(layout_price) layout_price from dws_db_prd.dws_newest_layout where dr = 0 and layout_price is not null and newest_id = '"+newest_id+"'")
    ...
    res,columnNames = con.query("select id,ori_id ,ori_table ,imei ,city_id ,county_id ,newest_name ,visit_month ,visit_date ,pv ,source ,idate ,newest_id ,create_date ,create_user ,update_date ,update_user ,current_week \
                        from dwb_db.a_dwb_customer_browse_log where visit_month<= '"+end_date_month+"' and visit_month >= '"+start_date_month+"' and newest_id = '"+newest_id+"'")
