...
"""
import configparser,getopt,os,sys,pymysql,pandas as pd,numpy as np,time, re
...
newest_imei=con.query("SELECT newest_id,imei,visit_month FROM dwb_db.dwb_customer_lookest_list_m where newest_id = '"+newest_id+"' and visit_month>='"+start_date+"' and visit_month<='"+end_date+"'")
imeis = "'"+re.sub(r',','\', \'',','.join(newest_imei['imei'].values.tolist()))+"'"
...
#                                   urgent         迫切
#                                   cre            增存
browse_tag = con.query('''select imei,concern,intention,urgent,cre from dws_db_prd.dws_imei_browse_tag where imei in ('''+imeis+''') and period = "'''+date_quarter+'''"''')
# newest_imei = newest_imei[newest_imei['newest_id'] == 'f2a7dbbdf18c0add9368c78fecef7277']
...
# 未知原因客户浏览表客户增加,为防止这种情况,活跃用户等于关注人数-增量人数
df1['retained'] = df1['intention']-df1['increase']
