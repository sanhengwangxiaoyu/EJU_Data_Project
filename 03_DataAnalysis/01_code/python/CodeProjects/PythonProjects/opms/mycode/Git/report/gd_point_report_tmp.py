...
class MysqlClient:
    ...
    def close(self):
        self.conn.close()
class Write_MysqlClient:
    def __init__(self, write_db_host,write_database,write_user,write_password):
        """
        create connection to hive server
        """
        self.conn = pymysql.connect(host=write_db_host, user=write_user,password=write_password,database=write_database,charset="utf8")
        # 更新SQL
...
conn = Write_MysqlClient(write_db_host,write_database,write_user,write_password) #链接读取的数据库

conn.updata_one("delete from event_tracking.dwb_et_user_browse_log where substr(create_time,1,10) = '"+yesterday_date_time+"'")
conn.updata_one("delete from event_tracking.dwb_et_user_invite_cc where substr(create_time,1,10) = '"+yesterday_date_time+"'")

conn.updata_one("delete from event_tracking.dwb_et_user_region where substr(create_time,1,10) = '"+yesterday_date_time+"'")
...
company_name = con.query("select t1.user_id,t2.name company_name from (select user_id,company_id from odsdb.ods_gd_points_goingdata_auth_user) t1 left join (select company_id,name from odsdb.ods_gd_points_goingdata_auth_company) t2 on t1.company_id = t2.company_id")
...
df_user =  pd.merge(df_user,company_name,how='left',on=['user_id'])
...
df_user['login_date'] = df_user[['login_date']].astype(str)
# df_user = df_user[df_user['user_type']!=1]
# df_user = df_user[df_user['login_date'].str[0:10]>='2021-12-06']
...
##dwb_et_user_invite_cc
#过滤掉内部用户和老用户
df_invite_used = auth_user_invite_used[(auth_user_invite_used['register_user_id'].isin(df_user['user_id']))]
...
##dwb_et_user_browse_log
#截取列
df_browse_log_user = df_user[['user_id','register_mobile']]#获取内部用户的用户和手机号码
#过滤掉内部用户的浏览记录
...
conn.updata_one("delete from event_tracking.dwb_et_day_report where substr(create_time,1,10) = '"+yesterday_date_time+"'")
conn.updata_one("delete from event_tracking.dwb_et_funnel where substr(create_time,1,10) = '"+yesterday_date_time+"'")
conn.updata_one("delete from event_tracking.dwb_et_user_action_count where substr(create_time,1,10) = '"+yesterday_date_time+"'")
conn.updata_one("delete from event_tracking.dwb_et_user_log_order where substr(create_time,1,10) = '"+yesterday_date_time+"'")
...
##dwb_et_funnel
#过滤掉内部用户
df_et_funnel = df_user[['user_id','register_mobile','login_date']]
...
tra_order = tra_order[(tra_order['user_id'].isin(df_user['user_id']))]#过滤掉内部用户和老用户
...
df_browse_userlog = pd.merge(df_user,df_log_order_user_log,how='inner',on=['user_id'])#浏览表关联出新的外部用户的地理
df_order_userlog = pd.merge(df_user,tra_order,how='inner',on=['user_id'])#交易表关联出新的外部用户的地理
#过滤掉城市为空的
...
df_othor_user_invite_used = auth_user[(~auth_user['user_id'].isin(auth_user_invite_used['register_user_id']))][['user_id']]#找出未通过邀请码新增的用户
...
df_etday_report = auth_user_invite_used[['invite_number','register_user_id']].append(df_othor_user_invite_used) ##合并到从邀请码表中读取的数据
...
# #合并邀请渠道
# df_report = pd.merge(auth_user_invite_used[(auth_user_invite_used['register_user_id'].isin(df_user['user_id']))].rename(columns={'register_user_id':'user_id'}),df_day_report,how='right',on=['user_id'])#合并标签
...
#和当天登录时间做合并
df_action_pages = pd.merge(df_action_pages,df_user,how='inner',on=['register_mobile'])#获取外部用户的id
...
if len(df_action_pages) != 0 :
    ...
    df_action_time['avg_url_num']=(df_action_time['pv']/df_action_time['user_id']).astype(int)
    # 查看报告跳出率
    df_user['date_time'] = yesterday_date_time
    df_action_new = df_user.groupby(['date_time'])['user_id'].count().reset_index()  ## 总的注册数量
...
#In[]
# df_report_result.to_csv(r'C:\Users\86133\Desktop\df_user.csv')

conn.updata_one("DELETE from event_tracking.dwb_et_user_region where substr(create_time,1,10) = '"+yesterday_date_time+"' and SUBSTR(login_date,1,10) > '"+yesterday_date_time+"'")

# conn.updata_one("insert into event_tracking.tmp_company select user_id,company_name name from event_tracking.dwb_et_user_region")
