...
for date_quarter in ['2021Q4']:
  ...
  ori_1 = con.query("select newest_id,imei from dwb_db.dwb_customer_lookest_list_m where newest_id = '"+newest_id+"' and visit_month = '"+start_date+"'and dr = 0")
  ori_2 = con.query("select newest_id,imei from dwb_db.dwb_customer_lookest_list_m where newest_id = '"+newest_id+"' and visit_month = '"+str(int(start_date)+1)+"'and dr = 0")
  ori_3 = con.query("select newest_id,imei from dwb_db.dwb_customer_lookest_list_m where newest_id = '"+newest_id+"' and visit_month = '"+end_date+"'and dr = 0")
  ori = ori_2.append(ori_1,ignore_index=True)
  ori = ori_3.append(ori,ignore_index=True)
  ...
  #                                                      city_id         城市id  
  #                                                      county_id       区县id  
  admit=con.query("select newest_id from dws_db_prd.dws_newest_period_admit where dr = 0 and period='"+date_quarter+"' and newest_id = '"+newest_id+"'")
  ...
  ime = con.query(" SELECT imei,min(add_new_code) cre,visit_month period FROM dwb_db.dwb_customer_add_new_code where imei in ("+imeis+") and dr = '0' and visit_quarter = '"+date_quarter+"' group by imei,visit_month")


  # In[6]:
  # 获取楼盘的城市和区县，以及浏览基本情况#
  # ime[['imei']]=ime['imei'].astype(int)
  # ori[['imei']]=ori['imei'].astype(int)
  df = pd.merge(ime,ori ,how='left' ,on=['imei'])
  df = pd.merge(admit, df, how='inner', on=['newest_id'])
  # 修改列名
  df = df[['city_id','newest_id','period','imei','cre']]
  df.columns = ['city','newest_id','visit_month','imei','cre']
