...
table_name = 'dwb_newest_city_state_num' # 要插入的表名称
database = 'dwb_db'
...
newest_city=con.query("select newest_id,city_id,city_name,sales_state,unit_price from dwb_db.dwb_newest_admit_info where dr = 0 and period='"+date_quarter+"'")


# In[3]:
#获取均价
newest_city[['unit_price']] = newest_city[['unit_price']].astype('int')
df1 = newest_city.groupby(['city_id','city_name'])['unit_price'].mean().astype('int').reset_index()
#计算各状态项目数量
df2 = newest_city.groupby(['sales_state','city_id','city_name'])['newest_id'].count().reset_index()
# 统计各个状态的楼盘数量
...
# 合并拼接
df3 = pd.merge(df1, df2_for_sale, how='left', on=['city_id'])
df3 = pd.merge(df3, df2_on_sale, how='left', on=['city_id'])
# 空为0
...
result = df3[['city_id','city_name_x','for_sale','on_sale','total_count','unit_price']]
result.columns = ['city_id','city_name','for_sale','on_sale','total_count','unit_price']
