...
# -*- coding: utf-8 -*-
"""
Created on Aug 26 09:44:47 2021
   切分楼盘表里的动态到odsdb库里
"""
import configparser
import os
...
table_name = 'ori_newest_provide_sche'
database = 'odsdb'
...
ori=con.query('''SELECT id housing_id,uuid,newest_name,provide_sche FROM dwb_db.dim_housing WHERE provide_sche NOT IN ('') AND provide_sche IS NOT NULL''')
...
#去掉标题太长，和正文太短的
# 计算长度
sche_result['provide_sche1'] = sche_result['provide_sche1'].apply(lambda x: sche_result['provide_title'] if x is None else x)
...
sche_result_7_out = sche_result_7_out[sche_result_7_out['provide_title_num'] < 25 ]
#合并
sche_result = pd.concat([sche_result_2,sche_result_7_out])
sche_result
# 去字段
ori_short = ori[['uuid','housing_id','newest_name']]
sche_result = pd.merge(sche_result,ori,how='left',on=['uuid'])
sche_result['sche_tag'] = sche_result['period']
sche_result['provide_date'] = sche_result['date']
sche_result['create_time'] = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) 
sche_result['update_time'] = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) 
sche_result = sche_result[['uuid','newest_name','period','provide_title','provide_date','provide_sche1','date','create_time','update_time','housing_id']]


# In[6]:
# 去重
sche_result = sche_result.groupby(['provide_sche1'])['uuid','newest_name','period','provide_title','provide_date','date','create_time','update_time','housing_id'].max().reset_index()
sche_result = sche_result[['uuid','newest_name','period','provide_title','provide_date','provide_sche1','date','create_time','update_time','housing_id']]
# 写入表中
sche_result.columns=['url','newest_name','sche_tag','provide_title','provide_date','provide_sche','date_clean','create_time','update_time','housing_id']
# 去除标题的问号
